 ...
 *
 */
public class AnyExpertSearcher extends AnySearcherBase implements AnySearcher {
		...
		try {
			...
			if(hitsPerPage < totalRecords) {
				...
				for(int i = 0; i < docs.length; i++) {
					...
					Record record = documentToRecord(hits.doc(docs[i]), schema.getColumns());
					records.add(record);
				}
			...
			if(reverse) {
				for(int i = reader.maxDoc() - 1; i >= 0; i--) {
					if(!reader.isDeleted(i)) {					
						if(n >= start && n < end) {
							records.add(AnySearcherBase.documentToRecord(reader.document(i), 
									schema.getColumns()));
						}
