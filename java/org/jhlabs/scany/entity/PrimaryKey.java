...
public class PrimaryKey {
	 ...
	 */
	public PrimaryKey(String primaryKey, String keyPattern) throws PrimaryKeyException {
		this();
	 ...
	 * @throws MultipartRequestzException
	 */
	public PrimaryKey(String keyPattern) {
		this();
	 ...
	 * @throws MultipartRequestzException
	 */
	public PrimaryKey(Schema schema) {
		this();
	...
	public void setKeyValue(String keyName, String keyValue) {
		...
		if(!wildcard) {
			for(int i = 0; i < ScanyConfig.WILDCARDS.length; i++) {
				if(keyValue.indexOf(ScanyConfig.WILDCARDS[i]) != -1) {
					wildcard = true;
					break;
				}
	...
	public String encode(String keyPattern) throws PrimaryKeyException {
		try {
			String[] keyNames = StringUtils.split(keyPattern, ScanyConfig.KEY_DELIMITER);
			...
			if(wildcard) {
				for(int i = 0; i < ScanyConfig.WILDCARDS.length; i++) {
					if(ScanyConfig.WILDCARDS[i].equals(keyValues[0].substring(0, 1)))
			...
			for(int i = 0; i < keyValues.length; i++) {
				...
				if(i < keyValues.length - 1)
					sb.append(ScanyConfig.KEY_DELIMITER);
			}
	...
	public void decode(String primaryKey) throws PrimaryKeyException {
		try {
			String[] keyNames = StringUtils.split(keyPattern, ScanyConfig.KEY_DELIMITER);
			String[] keyValues = StringUtils.split(primaryKey, ScanyConfig.KEY_DELIMITER);
