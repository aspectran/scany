...
public class WordSpaceAnalyzer {
	...
	public List analyze(String input)  throws MorphException {
		...
		for(int i=0;i<input.length();i++) {						
			...
			char[] f = SyllableRule.getFeature(input.charAt(i));
	...
	private int findJosaEnd(String snipt, int jstart) throws MorphException {
		...
		for(int i=jstart+1;i<snipt.length();i++) {
			char[] f = SyllableRule.getFeature(snipt.charAt(i));
			if(f[SyllableRule.IDX_JOSA2]=='0') break;
			jend = i;				
		}
...
//		Collections.sort(candidates, new WSOuputComparator());
	...
	throws MorphException {
		...
		char[] feature =  SyllableRule.getFeature(ch);
		...
		for(int i=strlen-1;i>0;i--) {
			...
			feature =  SyllableRule.getFeature(eomi.charAt(0));		
	...
	private int findEomiEnd(String snipt, int estart) throws MorphException {
		...
		for(int i=1;i<tail.length();i++) {
			char[] f = SyllableRule.getFeature(tail.charAt(i));	
			if(f[SyllableRule.IDX_EOGAN]=='0') break;
			start = i;				
		}
	...
	throws MorphException {
		...
		char[] chrs = po!=null&&po.getStem().length()>0 ? MorphRule.decompose(po.getStem().charAt(po.getStem().length()-1)) : null;
		...
		char[] f = nEnd<input.length() ? SyllableRule.getFeature(input.charAt(nEnd)) : null;			
	...
	private boolean findNounWithinStr(String str, int ws, int es) throws MorphException {
		...
		for(int i=es;i<str.length();i++) {
			char[] f = SyllableRule.getFeature(str.charAt(i));	
