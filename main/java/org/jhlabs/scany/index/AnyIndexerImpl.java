...
public class AnyIndexerImpl implements AnyIndexer {
	 ...
	 * @throws AnyIndexException
	 */
	public void insert(Record record, boolean duplicable) throws AnyIndexException {
		if(!duplicable) {
	...
	public void insert(Record record) throws AnyIndexException {
		try {
			// Primary Key 생성
			PrimaryKey primaryKey = new PrimaryKey(record.getPrimaryKey(), schema.getKeyPattern());
	...
	public void update(Record record) throws AnyIndexException {
		...
		try {
			// Primary Key 생성
			primaryKey = new PrimaryKey(record.getPrimaryKey(), schema.getKeyPattern());
		} catch(PrimaryKeyException e) {
			throw new AnyIndexException("유효한 Primary Key가 아닙니다.", e);
		}
	 ...
	 * 색인수정.
	 * 해당 레코드를 직접 수정한다.
	 * 수정 후 record의 Primary Key에는 실제 값이 담기게 된다. 
	 * @param record 수정 대상 레코드. 반드시 Primary Key값을 보유하고 있어야 한다.
	 * @throws AnyIndexException
	 */
	public void update(PrimaryKey primaryKey, Record record) throws AnyIndexException {
		try {
			if(primaryKey.hasWildcard())
				throw new IllegalArgumentException("PrimaryKey가 와일드카드 문자를 포함하고 있습니다.");
			...
			Document document = recordToDocument(record); 
			Term term = new Term(ScanyConfig.PRIMARY_KEY, record.getPrimaryKey());
	...
	public void delete(PrimaryKey primaryKey) throws AnyIndexException {
		...
		try {
			...
			Term term = new Term(ScanyConfig.PRIMARY_KEY, pkey);
			...
			if(whatQuery == 0) {
				...
				synchronized(directory) {
					...
					for(int i = 0; i < hits.length(); i++) {
						doc = hits.doc(i);
						term = new Term(ScanyConfig.PRIMARY_KEY, doc.get(ScanyConfig.PRIMARY_KEY));
	 ...
	 * @throws AnyIndexException
	 */
	public boolean exists(String primaryKey) throws AnyIndexException {
		IndexSearcher searcher = null;

		try {
			searcher = new IndexSearcher(directory);

			Term term = new Term(ScanyConfig.PRIMARY_KEY, primaryKey);
	...
	private Document recordToDocument(Record record) throws AnyIndexException {
		try {
			...
			// Primary Key 필드
			field = new Field(ScanyConfig.PRIMARY_KEY, record.getPrimaryKey(), Field.Store.YES,
