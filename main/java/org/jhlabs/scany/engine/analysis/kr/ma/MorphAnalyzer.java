...
public class MorphAnalyzer {
	...
	private void analysisByRule(String input, List candidates) throws MorphException {
		...
		for(int i = strlen - 1; i > 0; i--) {
			...
			char[] feature = Dictionary.getSyllableFeature(eomi.charAt(0));
	...
	private void addSingleWord(String word, List<AnalysisOutput> candidates) throws MorphException {
		...
		WordEntry entry;
		if((entry = Dictionary.getWord(word)) != null) {
	...
	public void analysisWithJosa(String stem, String end, List candidates) throws MorphException {
		...
		if(!Dictionary.existJosa(end) || (chrs.length == 3 && ConstraintRule.isTwoJosa(end))
				|| (chrs.length == 2 && (ConstraintRule.isThreeJosa(end)) || "".equals(end)))
		...
		WordEntry entry = Dictionary.getWordExceptVerb(stem);
		if(entry != null) {
			output.setScore(AnalysisOutput.SCORE_CORRECT);
	...
	public void analysisWithEomi(String stem, String end, List candidates) throws MorphException {
		...
		try {
			...
			WordEntry entry = Dictionary.getVerb(o.getStem());
			if(entry != null
					&& !("을".equals(end) && entry.getFeature(WordEntry.IDX_REGURA) == IrregularRule.IRR_TYPE_LIUL)) {
				AnalysisOutput output = o.clone();
				output.setScore(AnalysisOutput.SCORE_CORRECT);
				MorphRule.buildPtnVM(output, candidates);

				char[] features = Dictionary.getSyllableFeature(stem.charAt(stem.length() - 1)); // ㄹ불규칙일 경우
				if(features[SyllableRule.IDX_YNPLN] == '0' || morphs[1].charAt(0) != 'ㄴ')
					return;
			}

			String[] irrs = IrregularRule.restoreIrregularVerb(o.getStem(),
			...
			if(irrs != null) { // 불규칙동사인 경우
				...
				//				entry = Dictionary.getVerb(output.getStem());
			...
			if(VerbRule.ananlysisNSM(o.clone(), candidates))
				return;

			if(VerbRule.ananlysisNSMXM(o.clone(), candidates))
				return;

			// [체언 + '에서/에서부터' + '이' +  어미]
			if(VerbRule.ananlysisNJCM(o.clone(), candidates))
				return;

			if(VerbRule.analysisVMCM(o.clone(), candidates))
				return;
	...
	public boolean confirmCNoun(AnalysisOutput o) throws MorphException {
		...
		WordEntry cnoun = Dictionary.getCNoun(o.getStem());
	...
	private boolean constraint(AnalysisOutput o) throws MorphException {
		...
		if("화해".equals(cnouns.get(cnouns.size() - 1).getWord())) {
			if(!ConstraintRule.canHaheCompound(cnouns.get(cnouns.size() - 2).getWord()))
				return false;
		} else if(o.getPatn() == PatternConstants.PTN_NSM) {
			if("내".equals(o.getVsfx()) && cnouns.get(cnouns.size() - 1).getWord().length() != 1) {
				WordEntry entry = Dictionary.getWord(cnouns.get(cnouns.size() - 1).getWord());
